...
export default function VoiceChannel({ channel }) {
  ...
  const handleRemoteStream = (remoteUid, stream) => {
    console.log('Setting up remote audio for:', remoteUid);

    // Remove existing audio element if any
    if (remoteAudiosRef.current[remoteUid]) {
      remoteAudiosRef.current[remoteUid].srcObject = null;
      remoteAudiosRef.current[remoteUid].remove();
    }
  ...
  const handleParticipantLeft = (remoteUid) => {
    console.log('Participant left:', remoteUid);
    if (remoteAudiosRef.current[remoteUid]) {
      remoteAudiosRef.current[remoteUid].srcObject = null;
      remoteAudiosRef.current[remoteUid].remove();
      delete remoteAudiosRef.current[remoteUid];
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-discord-dark-1">
      {/* Channel Header */}
          ...
          {participants.length === 0 ? (
            ...
            <div className="space-y-2">
              {participants.map((participant) => (
                <div
                  key={participant.uid}
                  className="flex items-center gap-3 p-2 rounded bg-discord-dark-3"
                >
                  ...
                  {participant.isMuted && (
                    <svg
                      className="w-4 h-4 text-discord-red"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
        ...
        <div className="flex gap-4">
          {!isConnected ? (
            <button
              ...
              className="bg-discord-green hover:bg-discord-green/80 text-white px-6 py-3 rounded-lg font-semibold transition-colors flex items-center gap-2"
            >
            ...
            <>
              <button
                ...
                className={`${
                  isMuted
                    ...
                    : 'bg-discord-dark-3 hover:bg-discord-dark-4'
              ...
              <button
                ...
                className="bg-discord-red hover:bg-discord-red/80 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
              >
                연결 끊기
              </button>
            </>
